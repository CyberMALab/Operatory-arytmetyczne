...
Operatory arytmetyczne służą do wykonywania operacji matematycznych na zmiennych. Pierwszym z nich jest użyty już w poprzednim rozdziale operator przypisania. Jego symbolem jest znak równości ‘=’ . Zapisuje on wartość podaną po prawej stronie równości, do zmiennej po lewej stronie równości. Po lewej stronie równania może znajdować się tylko L-wyrażenie a po prawej można użyć bezpośrednio podania wartości (np. liczby), inna zmienna, albo działanie arytmetyczne z użyciem operatorów podanych niżej. L-wyrażeniem jest to wyrażenie, dla którego legalnym jest wykonanie operacji przypisania. Zwykle naszym L-wyrażeniem będzie po prostu zmienna.  Warto pamiętać, że aby przypisać zmienną znakową, należy znak do zapisu umieścić w pojedynczym cudzysłowu np. ‘a’. Istnieje więcej operatorów przypisania, które przypisują od razu wynik działania arytmetycznego wywołanego na przypisywanej zmiennej. Ich opis znajduje się w tabeli w dalszej części tego rozdziału.
...
Kolejnymi instrukcjami są operatory dodawania, odejmowania, mnożenia i dzielenia. Ich użycie jest bardzo intuicyjne więc, ich użycie najlepiej pokazać na przykładzie.
...
***
[Poprzednia część](https://github.com/CyberMALab/Hello-World-czyli-procerudy-wej-cia-i-wyj-cia-proste-typy-zmiennych.git) | [Spis treści](https://github.com/CyberMALab/Wprowadzenie-do-programowania-w-j-zyku-ANSI-C.git) | [Następna część](https://github.com/CyberMALab/Comming-Soon.git)
***
